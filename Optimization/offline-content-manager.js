...
class OfflineContentManager {
    constructor() {
        ...
        this.CONTENT_VERSION = 'v3.2.6-2026-03-13-mobile-launch-splash-fix';
    ...
    showProgressAnimation(percent, cached, total) {
        // Remove any existing progress overlay if present
        const existingProgress = document.getElementById('offline-progress-overlay');
        if (existingProgress) existingProgress.remove();
        // No progress popup/card will be shown
    }
    
    /**
     * Hide progress animation
    showCompletionPopup(cached = null, failed = 0) {
        // Remove any existing completion popup if present
        const popup = document.getElementById('offline-completion-popup');
        if (popup) popup.remove();
        // Hide progress animation if still visible
        ...
        // No completion popup/card will be shown
    }
